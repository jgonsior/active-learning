...
class SmallCNN(object):
    ...
    def build_model(self, X):
        ...
        model = Sequential()
        model.add(
            Conv2D(32, (3, 3), padding="same", input_shape=input_shape, name="conv1")
        )
    ...
    def transform(self, X):
        ...
        output = [layer.output for layer in model.layers if layer.name == "dense1"][0]
        func = K.function(inp + [K.learning_phase()], [output])
        for i in range(int(X.shape[0] / self.batch_size) + 1):
            minibatch = X[
                i * self.batch_size : min(X.shape[0], (i + 1) * self.batch_size)
            ]
            list_inputs = [minibatch, 0.0]
