...
flags.DEFINE_string(
    "source_dir", "/tmp/toy_experiments", "Directory with the output to analyze."
)
flags.DEFINE_string("save_dir", "/tmp/active_learning", "Directory to save charts.")
...
def plot_results(all_results, score_method, norm, stand, sampler_filter):
    colors = {
        "margin": "gold",
        "uniform": "k",
        "informative_diverse": "r",
        "mixture_of_samplers-margin-0.33-informative_diverse-0.33-uniform-0.34": "b",
        "pred_expert_advice_trip_agg": "g",
    }
    labels = {
        "margin": "margin",
        "uniform": "uniform",
        "mixture_of_samplers-margin-0.33-informative_diverse-0.33-uniform-0.34": "margin:0.33,informative_diverse:0.33, uniform:0.34",
        "informative_diverse": "informative and diverse",
        "pred_expert_advice_trip_agg": "expert: margin,informative_diverse,uniform",
    }
    markers = {
        "margin": "None",
        "uniform": "None",
        "mixture_of_samplers-margin-0.33-informative_diverse-0.33-uniform-0.34": ">",
        "informative_diverse": "None",
        "pred_expert_advice_trip_agg": "p",
    }
    ...
    for k in sorted(all_results.keys()):
        ...
        if (
            isinstance(k, tuple)
            and k[fields["score_method"]] == score_method
...
def get_standardize(filename):
    return get_between(filename, "_stand_", filename[filename.rfind("_") :]) == "True"
...
def main(argv):
    ...
    charting_filepath = os.path.join(FLAGS.save_dir, FLAGS.dataset + "_charts.pdf")
    sampling_methods = FLAGS.sampling_methods.split(",")
    scoring_methods = FLAGS.scoring_methods.split(",")
    files = gfile.Glob(os.path.join(FLAGS.source_dir, FLAGS.dataset + "*/results*.pkl"))
    files = [
        f
        for f in files
        if (
            get_sampling_method(FLAGS.dataset, f) in sampling_methods
            and get_scoring_method(f) in scoring_methods
            and get_normalize(f) == FLAGS.normalize
            and get_standardize(f) == FLAGS.standardize
        )
    ]
