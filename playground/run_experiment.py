...
flags.DEFINE_string(
    "data_dir", "/tmp/data", "Directory with predownloaded and saved datasets."
)
flags.DEFINE_string(
    "max_dataset_size",
    "15000",
    ("maximum number of datapoints to include in data " "zero indicates no limit"),
)
flags.DEFINE_float(
    "train_horizon", "1.0", "how far to extend learning curve as a percent of train"
)
flags.DEFINE_string("do_save", "True", "whether to save log and results")
FLAGS = flags.FLAGS
...
def generate_one_curve(
    ...
    def select_batch(sampler, uniform_sampler, mixture, N, already_selected, **kwargs):
    ...
    n_batches = (
        int(np.ceil((train_horizon * train_size - seed_batch) * 1.0 / batch_size)) + 1
    )
...
def main(argv):
    ...
    if not gfile.Exists(FLAGS.save_dir):
        try:
            ...
            print(
                (
                    "WARNING: error creating save directory, "
    ...
    all_results = {}
    max_dataset_size = (
        None if FLAGS.max_dataset_size == "0" else int(FLAGS.max_dataset_size)
    )
    ...
    for c in confusions:
        for m in mixtures:
            for seed in range(starting_seed, starting_seed + FLAGS.trials):
                ...
                if (
                    FLAGS.select_method == "None"
                    or FLAGS.select_method == FLAGS.score_method
                ):
                    select_model = None
                else:
    ...
    if do_save:
        ...
        filename = os.path.join(
            save_dir, filename + "_" + str(1000 + len(existing_files))[1:] + ".pkl"
        )
